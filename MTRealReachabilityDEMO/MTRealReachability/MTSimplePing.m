...
- (void)didFailWithError:(NSError *)error {
    id<MTSimplePingDelegate>  strongDelegate;
    ...
    if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(mtsimplePing:didFailWithError:)] ) {
        [strongDelegate mtsimplePing:self didFailWithError:error];
    }
}
...
- (void)sendPingWithData:(NSData *)data {
    ...
    if ( (bytesSent > 0) && (((NSUInteger) bytesSent) == packet.length) ) {
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(mtsimplePing:didSendPacket:sequenceNumber:)] ) {
            [strongDelegate mtsimplePing:self didSendPacket:packet sequenceNumber:self.nextSequenceNumber];
        }
    } else {
        NSError *   error;
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(mtsimplePing:didFailToSendPacket:sequenceNumber:error:)] ) {
            [strongDelegate mtsimplePing:self didFailToSendPacket:packet sequenceNumber:self.nextSequenceNumber error:error];
        }
    }
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber] ) {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(mtsimplePing:didReceivePingResponsePacket:sequenceNumber:)] ) {
                [strongDelegate mtsimplePing:self didReceivePingResponsePacket:packet sequenceNumber:sequenceNumber];
            }
        } else {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(mtsimplePing:didReceiveUnexpectedPacket:)] ) {
                [strongDelegate mtsimplePing:self didReceiveUnexpectedPacket:packet];
            }
        }
    } else {
...
static void SocketReadCallback(CFSocketRef s, CFSocketCallBackType type, CFDataRef address, const void *data, void *info) {
    ...
    obj = (__bridge MTSimplePing *) info;
    assert([obj isKindOfClass:[MTSimplePing class]]);
...
- (void)startWithHostAddress {
    ...
    if (err != 0) {
        ...
        strongDelegate = self.delegate;
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(mtsimplePing:didStartWithAddress:)] ) {
            [strongDelegate mtsimplePing:self didStartWithAddress:self.hostAddress];
        }
    }
    assert(fd == -1);
}
